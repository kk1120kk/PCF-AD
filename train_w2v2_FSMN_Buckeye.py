...
# -*- coding: utf-8 -*-
"""
Wav2Vec2 特征 + DeepFSMN 训练脚本（新增测试集评估、制表格式日志）
"""
...
from model_deep_fsmn import DeepFSMN
...
prename = "w2v2_HIDDEN=10_LR=1e-4_dfsmn_win=10_dropout=05_layer=4"

SAVE_ROOT = "hk_train_ckpt_Buckeye_new_split3"
...
def create_log_file(log_path, config_dict):
    with open(log_path, 'w', encoding='utf-8') as f:
        f.write("DeepFSMN on Wav2Vec2 Features - Training Log\n")
        f.write(f"Timestamp: {timestamp}\n")
        f.write("Configuration:\n")
        for key, value in config_dict.items():
            f.write(f"  {key}: {value}\n")
        f.write("\n")
        # 写表格头，固定宽度，列名靠左/靠右可自行调整
...
def log_epoch(log_path, epoch, train_loss, val_loss, train_acc, val_acc, test_acc, epoch_time):
    with open(log_path, 'a', encoding='utf-8') as f:
        # 每列宽度：epoch 5位，loss 10位（含小数点），acc 8位，时间 8位
...
def evaluate(model, data_loader, criterion, device):
    ...
    for padded_feat, padded_label, lengths in tqdm(data_loader, desc="Evaluating", leave=False):
        ...
        feats = padded_feat.transpose(1, 2)       # (B, 768, T)
        ...
        total += mask.sum().item()
        total_loss += loss.item() * mask.sum().item()   # 只计算有效帧损失
...
def main():
    ...
    pretrain_path = config.get('pretrain_ckpt', '')
