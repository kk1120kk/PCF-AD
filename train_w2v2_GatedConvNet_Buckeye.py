...
# -*- coding: utf-8 -*-
"""
Wav2Vec2 + GatedConvNet 训练脚本 (自动生成)
"""
import os
import datetime
import time
...
prename = "gatedconvnet_w2v2_Buckeye"
...
def create_log_file(log_path, config_dict):
    with open(log_path, 'w', encoding='utf-8') as f:
        f.write("Wav2Vec2 + GatedConvNet - Training Log\n")
...
def main():
    ...
    # ---------- 模型、损失、优化器 ----------
    model = GatedConvNet1D(input_dim=config['input_dim'], num_classes=config['num_classes'], hidden_dim=config['hidden_dim'], num_layers=config['num_layers'], kernel_size_1d=config['kernel_size_1d'], kernel_size_gated=config['kernel_size_gated'], dropout=config['dropout']).to(device)
