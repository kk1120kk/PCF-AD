...
from compare_model_TDNN import TDNN
...
def create_log_file(log_path, config_dict):
    with open(log_path, 'w', encoding='utf-8') as f:
        f.write("Wav2Vec2 + TDNN - Training Log\n")
...
def main():
    ...
    # ---------- 模型、损失、优化器 ----------
    model = TDNN(input_dim=config['input_dim'], num_classes=config['num_classes'], hidden_dim=config['hidden_dim'], num_layers=config['num_layers'], dropout=config['dropout']).to(device)
