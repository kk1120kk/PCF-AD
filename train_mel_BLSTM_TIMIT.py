...
# -*- coding: utf-8 -*-
"""
80 维 FBank (+Δ+Δ²) + BLSTM 训练脚本 —— TIMIT 数据集
"""
import os
import datetime
import time
...
def create_log_file(log_path, config_dict):
    with open(log_path, 'w', encoding='utf-8') as f:
        f.write("80FBank + BLSTM (TIMIT) - Training Log\n")
...
def main():
    ...
    model = BLSTM(input_dim=config['input_dim'], num_classes=config['num_classes'],
    ...
    for epoch in range(config['num_epochs']):
        best_acc, train_loss, val_loss, train_acc, val_acc, test_acc = train_one_epoch(
            epoch=epoch, model=model, train_loader=train_loader, val_loader=val_loader,
            test_loader=test_loader, optimizer=optimizer, criterion=criterion,
            best_acc=best_acc, save_path=SAVE_SUBDIR, save_freq=SAVE_FREQ,
            device=device, log_path=LOG_PATH, config_dict=config)
