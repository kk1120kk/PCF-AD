...
"""
BLSTM PPG 推理脚本
功能：加载训练好的 BLSTM 模型，分别为训练/验证/测试集生成帧级后验概率 (T, 40) 并保存为 .npy
用法：
  1. 修改下方配置区的模型路径、数据集目录和模型超参数
  2. 运行脚本
注意：BLSTM 模型输入为 (B, T, D)，特征文件应为 (T, 768) 的 .npy 文件，推理时无需转置
"""
...
import numpy as np
from compare_model_BLSTM import BLSTM    # 请确保模型文件在当前目录

# ======================== 配置区（请根据实际情况修改） ========================

# 1. 模型检查点路径（训练得到的最佳 .pth）
MODEL_CKPT = "/root/autodl-tmp/phn_ASR_2/hk_compare/blstm_wavlm_TIMIT_20260801_091249/best_epoch24_valacc85.99_20260801_091249.pth"

# 2. 数据集特征目录（输入）与后验输出目录（独立配置）
# ----- 训练集 -----
TRAIN_FEAT_DIR   = "/root/autodl-tmp/phn_ASR_2/TIMIT_data/wavlm_hidden_states/layer_12/TRAIN/feat"
TRAIN_OUTPUT_DIR = "/root/autodl-tmp/phn_ASR_2/hk_compare_get_PPG/BLSTM_wavlm_TIMIT/train"

# ----- 验证集 -----
VAL_FEAT_DIR     = "/root/autodl-tmp/phn_ASR_2/TIMIT_data/wavlm_hidden_states/layer_12/VAL/feat"
VAL_OUTPUT_DIR   = "/root/autodl-tmp/phn_ASR_2/hk_compare_get_PPG/BLSTM_wavlm_TIMIT/val"

# ----- 测试集 -----
TEST_FEAT_DIR    = "/root/autodl-tmp/phn_ASR_2/TIMIT_data/wavlm_hidden_states/layer_12/TEST/feat" 
TEST_OUTPUT_DIR  = "/root/autodl-tmp/phn_ASR_2/hk_compare_get_PPG/BLSTM_wavlm_TIMIT/test"

# 3. 模型超参数（必须与训练时完全一致）
INPUT_DIM = 768
NUM_CLASSES = 40
HIDDEN_DIM = 512            # BLSTM 单向隐层维度
NUM_LAYERS = 3
DROPOUT = 0.5
...
def inference_and_save(feat_dir, output_dir, model, device):
    ...
    with torch.no_grad():
        for idx, fname in enumerate(feat_files):
            ...
            # BLSTM 输入要求 (B, T, D)，直接 unsqueeze 即可
            feat_tensor = feat_tensor.unsqueeze(0)                 # (1, T, 768)
