...
# -*- coding: utf-8 -*-
"""
GatedConvNet1D PPG 推理脚本
功能：加载训练好的 GatedConvNet1D 模型，分别为训练/验证/测试集生成帧级后验概率 (T, 40) 并保存为 .npy
用法：
...
import numpy as np
from compare_model_gateconv import GatedConvNet1D    # 请确保模型文件在当前目录

# ======================== 配置区（请根据实际情况修改） ========================

# 1. 模型检查点路径（训练得到的最佳 .pth）
MODEL_CKPT = "/root/autodl-tmp/phn_ASR_2/hk_compare/gatedconvnet_wavlm_Buckeye_20260731_200008/best_epoch33_valacc79.36_20260731_200008.pth"
...
TRAIN_OUTPUT_DIR = "/root/autodl-tmp/phn_ASR_2/hk_compare_get_PPG/GateConv_wavlm_Buckeye/train"

# ----- 验证集 -----
VAL_FEAT_DIR     = "/root/autodl-tmp/phn_ASR_2/Buckeye3_hk_clean_train_val_test_70_15_15/wavlm_hidden_states/layer_12/val/feat"
VAL_OUTPUT_DIR   = "/root/autodl-tmp/phn_ASR_2/hk_compare_get_PPG/GateConv_wavlm_Buckeye/val"

# ----- 测试集 -----
TEST_FEAT_DIR    = "/root/autodl-tmp/phn_ASR_2/Buckeye3_hk_clean_train_val_test_70_15_15/wavlm_hidden_states/layer_12/test/feat"
TEST_OUTPUT_DIR  = "/root/autodl-tmp/phn_ASR_2/hk_compare_get_PPG/GateConv_wavlm_Buckeye/test"

# 3. 模型超参数（必须与训练时完全一致）
INPUT_DIM = 768
NUM_CLASSES = 40
HIDDEN_DIM = 512            # GatedConvNet 的 hidden_dim
NUM_LAYERS = 10
KERNEL_SIZE_1D = 7
KERNEL_SIZE_GATED = 2
DROPOUT = 0.5               # 此处 Dropout 仅用于初始化，推理时会被 eval 模式关闭
