...
# -*- coding: utf-8 -*-
"""
80 维 FBank (+Δ+Δ²) + DeepFSMN 训练脚本 —— Buckeye 数据集
"""
import os
import datetime
import time
...
from compare_model_DFSMN import DeepFSMN    # 确保类名为 DeepFSMN

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
prename = "mel_dfsmn_Buckeye"
...
# ... (日志和评估函数与 BLSTM 版本结构一致，仅需调整 evaluate/train 中的特征处理)
# 为节省篇幅，以下只给出关键差异部分，完整代码可沿用模板。

# 注意：DFSMN 模型输入需要 (B, D, T)，因此 feats = padded_feat.transpose(1, 2)
# 在 evaluate 和 train_one_epoch 中替换相应行为。

# --------- 完整代码 ---------
# (以下为完整脚本，为避免重复已折叠，实际使用时请展开)
def create_log_file(log_path, config_dict):
    with open(log_path, 'w', encoding='utf-8') as f:
        f.write("80FBank + DeepFSMN (Buckeye) - Training Log\n")
...
def evaluate(model, data_loader, criterion, device):
    ...
    for padded_feat, padded_label, lengths in tqdm(data_loader, desc="Evaluating", leave=False):
        ...
        feats = padded_feat.transpose(1, 2)   # (B, D, T)
        logits = model(feats)
...
def main():
    config = {
        'train_feat_dir': "/root/autodl-tmp/phn_ASR_2/Buckeye3_hk_clean_train_val_test_70_15_15/feat_80fbank_20ms/train",
        'train_label_dir': "/root/autodl-tmp/phn_ASR_2/Buckeye3_hk_clean_train_val_test_70_15_15/labels_20ms/train",
        'val_feat_dir': "/root/autodl-tmp/phn_ASR_2/Buckeye3_hk_clean_train_val_test_70_15_15/feat_80fbank_20ms/val",
        'val_label_dir': "/root/autodl-tmp/phn_ASR_2/Buckeye3_hk_clean_train_val_test_70_15_15/labels_20ms/val",
        'test_feat_dir': "/root/autodl-tmp/phn_ASR_2/Buckeye3_hk_clean_train_val_test_70_15_15/feat_80fbank_20ms/test",
