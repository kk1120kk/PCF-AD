"""
批量提取本地 WavLM 模型指定层特征，自动末尾填充以保证输出帧数与按 320 点分帧的标签一致。
输入：WAV_DIR 中的 .wav 文件（任意采样率，脚本会自动重采样至 16kHz 单声道）
输出：OUTPUT_DIR 中的同名 .npy 文件，形状 (原始长度//320, hidden_dim)
      特征来自第 HIDDEN_LAYER 层 Transformer（1~12 for Base, 1~24 for Large），
      或 HIDDEN_LAYER=0 提取 CNN 编码器输出。
"""
...
OUTPUT_DIR = "./Buckeye_hk_clean_train_val_test/wavlm_hidden_states/layer_12/VAL/feat"  # 输出 npy 目录
MODEL_PATH = "/root/autodl-tmp/phn_ASR_2/WavLM"                         # 本地 WavLM 模型路径
TARGET_SR = 16000                                   # WavLM 要求采样率 16kHz
HIDDEN_LAYER = 12                                   # 提取层索引: 0=CNN编码器, 1~12=第1~12层Transformer
# ===================================================
...
def main():
    ...
    # 加载本地 WavLM 模型 (transformers >= 4.20 推荐)
    model = transformers.WavLMModel.from_pretrained(MODEL_PATH)
    model.to(device)
    model.eval()
    ...
    layer_desc = "CNN 编码器" if HIDDEN_LAYER == 0 else f"第 {HIDDEN_LAYER} 层 Transformer"
    ...
    if not wav_files:
        ...
        return
    print(f"共发现 {len(wav_files)} 个文件，开始提取...")
